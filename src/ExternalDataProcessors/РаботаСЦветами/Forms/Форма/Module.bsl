...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Соответствие = ОбработкаОбъект().КаталогСочетанийЦветаТекстаИФона();
	Для Каждого КЗ Из Соответствие Цикл
		НоваяСтрока = ТаблицаСочетанияЦветов.Добавить();
		НоваяСтрока.ЦветТекста = КЗ.Ключ;
		НоваяСтрока.ЦветФона = КЗ.Значение;
	КонецЦикла;	
